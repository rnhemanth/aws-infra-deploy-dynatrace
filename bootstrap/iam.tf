locals {
  ...
  iam_role_name   = "${var.service}-sec-rol-github-deploy-${var.identifier}"
  tf_state_bucket = "${local.project_name}-plat-monitoring-s3-terraform-state-${data.aws_caller_identity.current.account_id}"
  ddb_table_name  = "${local.project_name}-plat-monitoring-s3-terraform-locks-${data.aws_caller_identity.current.account_id}"
...
resource "aws_iam_policy" "S3GitHubDeployerPolicy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      ##### spliting deletion from above to a specific bucket location
      {
        Action = [
          "s3:DeleteBucket",
          "s3:DeleteObject",
        ...
        Effect   = "Allow"
        Resource = [
          #"arn:aws:s3:::*-dynatrace-s3-bucket-*",
          #"arn:aws:s3:::*-dynatrace-s3-bucket-*/*",
          "arn:aws:s3:::${local.tags.environment}-${local.tags.service}-s3-bucket-${data.aws_caller_identity.current.account_id}",
          "arn:aws:s3:::${local.tags.environment}-${local.tags.service}-s3-bucket-${data.aws_caller_identity.current.account_id}/*"
        ]
        Condition = {
          "StringEquals" = {
...
resource "aws_iam_policy" "SSMGitHubDeployerPolicy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      #   }
      # },
      #####Additional permissions for Actuvegate introduced.
      {
        Action = [
          ...
          "iam:TagInstanceProfile",
          "ec2:DescribeImages",
          #added 15/04/2025 due to updates in emis sre repo these perms are required for aws stream
